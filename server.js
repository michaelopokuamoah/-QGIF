...
// ============================================================
// ML MODEL LOADER — Random Forest from Google Colab Training
// ============================================================
let ML_MODEL = null;
try {
  // Try MLP model first (v2, 93% accuracy)
  const mlpPath = path.join(__dirname, 'qgif_mlp_js.json');
  const rfPath = path.join(__dirname, 'qgif_model_js.json');
  
  if (fs.existsSync(mlpPath)) {
    ML_MODEL = JSON.parse(fs.readFileSync(mlpPath, 'utf8'));
    console.log(`  ✓ ML Model loaded: MLP Neural Network (93% accuracy, ${ML_MODEL.layers.length} layers, ${ML_MODEL.n_features} features)`);
  } else if (fs.existsSync(rfPath)) {
    ML_MODEL = JSON.parse(fs.readFileSync(rfPath, 'utf8'));
    console.log(`  ✓ ML Model loaded: Random Forest (${ML_MODEL.n_estimators} trees, ${ML_MODEL.features.length} features)`);
  } else {
    console.log(`  ⚠ No ML model found — ML predictions disabled`);
  }
} catch (e) {
  console.log(`  ⚠ Could not load ML model: ${e.message}`);
}
...
// Predict using MLP Neural Network
function predictMLP(x_scaled) {
  let current = [...x_scaled];
  
  // Forward pass through hidden layers with ReLU
  for (let i = 0; i < ML_MODEL.layers.length - 1; i++) {
    const layer = ML_MODEL.layers[i];
  ...
  const outLayer = ML_MODEL.layers[ML_MODEL.layers.length - 1];
  let outSum = outLayer.biases[0];
  for (let k = 0; k < current.length; k++) {
    outSum += current[k] * outLayer.weights[k][0];
  }
  const miningProb = 1 / (1 + Math.exp(-outSum)); // Sigmoid
  return [1 - miningProb, miningProb]; // [not_mining, mining]
}

// Predict using Random Forest (legacy)
function predictTree(tree, x) {
  ...
  return value.map(v => v / sum);
}

function predictMining(satelliteIndices) {
  if (!ML_MODEL) return null;
  try {
    const featureMap = {
      ndvi_mean: satelliteIndices.ndvi_mean || 0,
      ndvi_p10: satelliteIndices.ndvi_p10 || 0,
      bsi_mean: satelliteIndices.bsi_mean || 0,
      bsi_change: satelliteIndices.bsi_change_from_baseline || 0,
      mndwi_mean: satelliteIndices.mndwi_mean || 0,
      ior_mean: satelliteIndices.iron_oxide_ratio || 0,
      cmr_mean: satelliteIndices.clay_mineral_ratio || 0,
      ndvi_change: satelliteIndices.ndvi_change_from_baseline || 0,
      degradation_gap: (satelliteIndices.ndvi_mean || 0) - (satelliteIndices.ndvi_p10 || 0),
      water_fraction: satelliteIndices.water_coverage_pct || 0,
    };

    const features = ML_MODEL.features || ML_MODEL.scaler?.features || Object.keys(featureMap);
    const x_raw = features.map(f => featureMap[f] || 0);

    // Scale features using stored scaler
    const mean = ML_MODEL.scaler.mean;
    const scale = ML_MODEL.scaler.scale;
    const x_scaled = x_raw.map((v, i) => (v - mean[i]) / scale[i]);

    let probs;

    if (ML_MODEL.type === 'MLP') {
      // Use MLP neural network
      probs = predictMLP(x_scaled);
    } else {
      // Use Random Forest (legacy)
      const accumProbs = [0, 0];
      for (const tree of ML_MODEL.trees) {
        const p = predictTree(tree, x_raw);
        accumProbs[0] += p[0];
        accumProbs[1] += p[1];
      }
      accumProbs[0] /= ML_MODEL.n_estimators;
      accumProbs[1] /= ML_MODEL.n_estimators;
      probs = accumProbs;
    }

    return {
      mining_probability_pct: Math.round(probs[1] * 1000) / 10,
      not_mining_probability_pct: Math.round(probs[0] * 1000) / 10,
      prediction: probs[1] > 0.5 ? 'MINING' : 'NOT MINING',
      confidence_pct: Math.round(Math.max(probs[0], probs[1]) * 1000) / 10,
      model: ML_MODEL.type === 'MLP' ? 'MLP Neural Network v2 (93% accuracy)' : 'Random Forest (Colab-trained)',
      accuracy_pct: ML_MODEL.accuracy || 69.2,
    };
  } catch (e) {
...
function interpretDetection(raw, radiusKm = 5) {
  ...
  const mlPrediction = predictMining(satIndices);
  ...
  return {
    ...
    ml_prediction: mlPrediction || { status: 'NOT_AVAILABLE', message: 'ML model not loaded' },
    water_contamination: {
...
// Stores detect-live results to avoid re-querying Earth Engine
// Cache expires after 7 days (Sentinel-2 revisit cycle is 5 days)
// ============================================================
const DETECTION_CACHE = new Map();
const CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 days

function makeCacheKey(lat, lng, radius) {
  return `${parseFloat(lat).toFixed(3)},${parseFloat(lng).toFixed(3)},${radius || 5}`;
}

function getFromCache(key) {
  const entry = DETECTION_CACHE.get(key);
...
function setInCache(key, data) {
  DETECTION_CACHE.set(key, { cached_at: Date.now(), data });
  // Prevent memory bloat — max 200 cached locations
  if (DETECTION_CACHE.size > 200) {
    const firstKey = DETECTION_CACHE.keys().next().value;
    DETECTION_CACHE.delete(firstKey);
  }
}
...
app.post('/detect-live', async (req, res) => {
  ...
  if (cached) {
    console.log(`  ✓ Cache HIT for ${locationName} — instant response`);
    return res.json({
      ...cached,
      location: locationName,
      timestamp: new Date().toISOString(),
      from_cache: true,
      cache_age_hours: Math.round((Date.now() - (DETECTION_CACHE.get(cacheKey)?.cached_at || Date.now())) / 3600000 * 10) / 10
    });
  }

  console.log(`  ⏳ Cache MISS for ${locationName} — querying Earth Engine...`);
...
All values are satellite-derived. Water contamination and health risk values are proxies calculated from spectral signatures, NOT direct chemical or clinical measurements.`,
    ...
    console.log(`  ✓ Cached result for ${locationName} (cache size: ${DETECTION_CACHE.size})`);
...
// Monitoring scheduler — DISABLED on startup to prevent blocking other requests
// Use POST /monitoring/run to trigger manually when needed
// setTimeout(() => {
